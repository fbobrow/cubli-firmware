...
// Surface parameters
const float b = 0;                       // Surface viscuous friction coefficient [N.m.s/rad]
...
const float bw = 1.06e-5;                // Rotational viscuous friction coefficient [N.m.s/rad]
...
const float omega_n_0 = sqrt(m_c_bar_g_l*sqrt(3.0)/(I_c_xx_bar-I_c_xy_bar));
const float omega_n_1 = b/(I_c_xx_bar+2*I_c_xy_bar);
const float beta = (I_c_xx_bar+2*I_c_xy_bar)/I_w_xx;
const float gamma = (I_c_xx_bar-I_c_xy_bar)/I_w_xx;
const float delta = m_c_bar_g_l*sqrt(3.0)/I_w_xx;
const float epsilon = b/I_w_xx;
